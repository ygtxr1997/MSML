...
class PartialFC(Module):
    ...
    def forward_backward(self, label, features, optimizer):
        ...
        # norm_weight is normalized
        # total_label:(batch, )
        # norm_weight:(93431 depart to {31144+31144+31143}, dim)
        ...
        # features:(batch_gpu, dim)
        # total_features:(batch, dim)
        ...
        with torch.no_grad():
            ...
            # get one-hot
            grad = logits_exp
            # print('total label', total_label.shape, total_label)
            index = torch.where(total_label != -1)[0]  # total:(batch=16*4), index:(varied=17/15/21)
            # print('index', index.shape, index)
            one_hot = torch.zeros(size=[index.size()[0], grad.size()[1]], device=grad.device)
            one_hot.scatter_(1, total_label[index, None], 1)  # one_hot:(varied, num_classes // nnode)
            # -> label smoothing
            epsilon = 0.1
            one_hot = (1 - epsilon) * one_hot
            one_hot[torch.where(one_hot == 0)] = epsilon / (one_hot.shape[1] - 1)
            # print('------label smooth', one_hot, torch.topk(one_hot, k=2, dim=1))
