...
class sphere(nn.Module):
    def __init__(self, type=20, is_gray=False, input_size=(112, 96)):
        ...
        if input_size == (112, 96):
            self.fc = nn.Linear(512 * 7 * 6, 512)  # for (112,96) input, github
        elif input_size == (112, 112):
            self.fc = nn.Linear(512 * 7 * 7, 512)  # for (112,112) input, issue
        else:
...
def cosface2018(input_size=(112, 96)):
    """
    input_size (H,W)
    """
    id_model = sphere(input_size=input_size).cuda()
    id_path = '/gavin/code/FaceSwapping/modules/third_party/cosface/ACC99.28.pth'  # 112x96
...
if __name__ == '__main__':
    net = cosface2018()
    img = torch.randn((5, 3, 112, 96)).cuda()
    pred = net(img)
    print(pred.shape)
