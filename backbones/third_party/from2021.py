...
class From2021(torch.nn.Module):
    def __init__(self):
        super(From2021, self).__init__()
        weight_path = '/gavin/code/MSML/backbones/third_party/weights/model_p5_w1_9938_9470_6503.pth.tar'
        checkpoint = torch.load(weight_path)
        state_dict = checkpoint['state_dict']

        num_mask = len(get_grids(112, 96, 5))
        model = LResNet50E_IR_Occ(num_mask=num_mask)
...
if __name__ == '__main__':
    net = From2021().cuda().eval()
    with torch.no_grad():
        img = torch.randn(12, 3, 112, 96).cuda()
        feat = net(img)
        print(feat.shape)
